  ...
  for (let [imageVariableSet, values] of Object.entries(imageVariables)) {
    for (let variable of values) {
      updateWMO(
        imageVariableSet,
        variable,
        data[imageVariableSet][variable],
        data[imageVariableSet]["is_day"]
      );
      updateWindDirection(
        imageVariableSet,
        variable,
  ...
  if (variableSet != "current") {
    for (let [index, element] of Array.from(elements).entries()) {
      if (variable == "sunrise" || variable == "sunset") {
        ...
        element.innerHTML =
          value[index].slice(8, 10) + "/" + value[index].slice(5, 7);
      } else if (variableSet == "hourly") {
        element.innerHTML =
          Math.round(parseInt(value[index * 3 + 2])) + " " + units;
      } else {
        element.innerHTML = Math.round(parseInt(value[index])) + " " + units;
      }
    }
    ...
    if (variableSet != "current") {
      for (let [index, element] of Array.from(elements).entries()) {
        if (variableSet == "hourly") {
          if (isDay[index * 3 + 2]) {
            element.src = `./assets/images/weather_icons/${
              value[index * 3 + 2]
            }d.png`;
          } else {
            element.src = `./assets/images/weather_icons/${
